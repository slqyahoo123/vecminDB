...
impl GRUProcessor {
    ...
    pub fn new(config: ContextAwareConfig) -> Self {
        ...
        let input_weights = Array2::zeros((input_dim, 3 * hidden_dim));
        let hidden_weights = Array2::zeros((hidden_dim, 3 * hidden_dim));
        let bias = Array1::zeros(3 * hidden_dim);
    ...
    fn forward(&self, x: &Array2<f32>) -> Array2<f32> {
        ...
        for t in 0..x.nrows() {
            ...
            let gates = gates.into_shape((self.config.hidden_dim, 3)).unwrap();
            
            let r = self.activation.apply(&gates.slice(ndarray::s![.., 0]));
            let z = self.activation.apply(&gates.slice(ndarray::s![.., 1]));
            let n = self.activation.apply(&gates.slice(ndarray::s![.., 2]));
            
            let ht = (1.0 - &z) * &ht_prev + &z * n;
...
impl ContextProcessor for GRUProcessor {
