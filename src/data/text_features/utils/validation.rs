...
pub fn validate_features(features: &[f32]) -> Result<()> {
    if features.is_empty() {
        return Err(Error::InvalidArgument("特征向量为空".to_string()));
    }
...
pub fn validate_json_data(data: &Value) -> Result<()> {
    ...
    if data.is_object() {
        ...
        if obj.is_empty() {
            return Err(Error::InvalidArgument("JSON对象为空".to_string()));
        }
    }
    ...
    if data.is_array() {
        ...
        if arr.is_empty() {
            return Err(Error::InvalidArgument("JSON数组为空".to_string()));
        }
    }
